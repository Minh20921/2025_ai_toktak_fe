...
import { LineThrough } from '@/app/(DashboardLayout)/rate-plan-old/icons';
import Image from 'next/image';
import { useSelector } from 'react-redux';
import { RootState } from '@/app/lib/store/store';
import { GiftFeature } from './gifts';
...
const PackageStandard = ({ handleClickBtn }: { handleClickBtn: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-10 right-10 w-[61px] h-[61px] flex items-center justify-center rounded-full bg-[#AA51FF]/10 backdrop-blur-[30px] shadow-[0_0_30px_2px_rgba(170,81,255,0.2)]">
        <Image src="/images/rate/tag-standard.png" alt="tag-standard" width={43} height={45} className="object-contain" objectFit="contain" />
      </div>

      <div className="flex flex-col items-start p-10 w-full text-[#6A6A6A]">
        <h2 className="font-bold text-2xl leading-[29px] group-hover:text-[#272727]">스탠다드</h2>
  ...
  group-hover:text-white group-hover:scale-[1.03] "
        ...
        >
          ...
          />
          <span className="relative z-10">Comming Soon</span>
        </button>
        <div className="mt-10">
          <PricingFeature>
            <span>콘텐츠 생성 개수</span>
