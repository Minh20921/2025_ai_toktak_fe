...
const RatePlanMobile = ({ tab = 0 }: { tab: number }) => {
  ...
  const initPackageList = [
    {
      ...
      finalPrice: 5000,
      description1: [
        '콘텐츠 생성<b> 15개</b>/월 <br>(상품 5개×영상/이미지/블로그)',
        '콘텐츠 다운로드<b>신규 가입 후 3일간 </b>무제한 가능',
        '업로드 방식 수동',
        '멀티링크 제공',
      ],
      description2: null,
      btnLabel: user ? '나의 현재 플랜' : '무료 이용하기',
      isDisabled: !!user,
      action: () => {
        if (!user) {
    ...
    },
    {
      ...
      finalPrice: 5000,
      description1: [
        '콘텐츠 생성 개수 <b>90개</b>/월(상품 30개×영상/이미지/블로그)',
        '생성 콘텐츠 종류 <b>3개</b>',
        'SNS 배포 채널<b> 1개</b>(선택)',
      ...
      description2: ['콘텐츠 다운로드 무제한'],
      description3: ['친구 추천 시 최대 90일 무료'],
      btnLabel:
        user?.subscription === 'BASIC' || user?.subscription === 'NEW_USER' ? '나의 현재 플랜' : '베이직 이용하기',
      action: () => window.open('https://domeggook.com/59856506', '_blank'),
    },
    {
      name: 'standard',
      title: '스탠다드',
      ...
      finalPrice: 5000,
      description1: [
        '콘텐츠 생성 개수<b> 180개</b>/월(상품 60개×영상/이미지/블로그)',
        '생성 콘텐츠 종류 <b>3개</b>',
        'SNS 배포 채널<b> 7개</b>(모두)',
        'SNS당 자동 게시 콘텐츠 수<b> 2개</b>',
        '업로드 방식<b> AI 자동</b>',
        '멀티링크 제공',
      ],
      description2: ['콘텐츠 다운로드 무제한', 'AI 바이럴 영상', 'AI 바이럴 문구'],
      description3: ['친구 추천 시 최대 90일 무료(베이직)'],
      btnLabel: user?.subscription === 'STANDARD' ? '나의 현재 플랜' : '스탠다드 이용하기',
      action: () => window.open('https://domeggook.com/59856506', '_blank'),
    },
  ];
  ...
  //         finalPrice: 5000,
  //         description1: [
  //           '콘텐츠 생성 <b>30개</b>/월 (상품 10개*콘텐츠 3종)',
  //           '생성된 콘텐츠 <b>다운로드 가능</b>',
  //           '배포 방식 <b>수동</b>',
  //         ],
  //         description2: null,
        ...
        : [
            {
              ...
              btnLabel: '컨설팅 신청',
              action: () => {
                window.open('https://forms.gle/3dgtzd7NvmfAePnQ8', '_blank');
              },
            },
          ],
    );
  ...
  return (
    <Box className="sm:hidden font-pretendard bg-[#fff] px-[20px] sm:bg-[#F8F8F8] min-h-screen w-full mx-auto sm:py-[40px]">
        ...
        >
          {packageList?.map((item: any, index: number) => (
            <Box key={item.name}>
              ...
              />
              {index === 1 && (
                <Box className="px-[10px] pt-[10px] d-none hidden sm:block">
                  ...
                  <Box className="flex justify-between w-full">
                    ...
                    <Box className="flex items-center">
                      <span className="text-[#4776EF] font-bold text-[16px] mr-1">
                        {formatNumberKR(2500 * addedSns)}원
                      </span>
                      /월
                    </Box>
                  </Box>
      ...
      <Box className="space-y-2 px-[20px] mt-[30px]">
        {packageList[selectedPackage].description1.map((item, index) => (
          <Box key={`description1_${index}`} className="flex items-center gap-2.5">
        ...
        {packageList[selectedPackage]?.description2 &&
          packageList[selectedPackage].description2?.map((item, index) => (
            <Box key={`description2_${index}`} className="flex items-center gap-2.5">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
        ...
        {packageList[selectedPackage]?.description3 &&
          packageList[selectedPackage].description3?.map((item, index) => (
            <Box key={`description2_${index}`} className="flex items-center gap-2.5">
